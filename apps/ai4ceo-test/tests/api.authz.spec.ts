// FR-7: /api/ai/tutor 권한 (AI-01, LMS-AI04, SEC-10). 계정 세션 쿠키로 직접 호출.
...
async function tutorStatus(account: AccountKey | null): Promise<number> {
  ...
  try {
    ...
    if (account) {
      ...
      headers["cookie"] = cookieHeader(SUPABASE_URL, session);
    }
